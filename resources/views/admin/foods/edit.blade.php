...
<div class="max-w-2xl">
    ...
    <div class="bg-white rounded-xl shadow-sm border border-gray-100 p-8">
        <h2 class="text-2xl font-bold text-gray-900 mb-6">Taomni Tahrirlash</h2>
        ...
        <form action="{{ route('admin.foods.update', $food) }}" method="POST" enctype="multipart/form-data" class="space-y-6">
            ...
            <div>
                ...
                <textarea id="description" name="description" rows="3"
                          class="w-full px-4 py-2 border-gray-300 rounded-lg shadow-sm border focus:border-amber-500 focus:ring-amber-500 transition @error('description') border-red-500 @enderror">{{ old('description', $food->description) }}</textarea>
                @error('description')
                <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                @enderror
            </div>
            ...
            <div>
                ...
                <input type="number" id="price" name="price" step="0.01" value="{{ old('price', $food->price) }}" required
            ...
            <!-- Image Upload -->
            <div>
                <label for="image" class="block text-sm font-medium text-gray-700 mb-2">Rasm</label>
                @if($food->image)
                    <div class="mb-3">
                        <img src="{{ asset($food->image) }}" alt="{{ $food->name }}"
                             class="h-24 w-24 object-cover rounded-lg border border-gray-200">
                        <p class="text-xs text-gray-500 mt-1">Hozirgi rasm. Yangi rasm yuklasangiz, o'zgaradi.</p>
                    </div>
                @endif
            ...
            <div>
                <label class="flex items-center gap-2 cursor-pointer">
                    <input type="checkbox" name="is_available" value="1" {{ old('is_available', $food->is_available) ? 'checked' : '' }} class="rounded border-gray-300 text-amber-600 focus:ring-amber-500">
