...
<div class="max-w-2xl">
    ...
    <div class="bg-white rounded-xl shadow-sm border border-gray-100 p-8">
        <h2 class="text-2xl font-bold text-gray-900 mb-6">Yangi Stol Qo'shish</h2>
        ...
        <form action="{{ route('admin.tables.store') }}" method="POST" class="space-y-6">
            ...
            <div>
                ...
                <input type="text" id="name" name="name" value="{{ old('name') }}" placeholder="Masa 1, VIP 3" required
            ...
            <div>
                ...
                <input type="number" id="capacity" name="capacity" value="{{ old('capacity') }}" min="1" max="50" required
            ...
            <div>
                ...
                <input type="text" id="location" name="location" value="{{ old('location') }}" placeholder="Ichki zal, Teras"
            ...
            <div>
                ...
                <select id="status" name="status" required
                    ...
                    <option value="available" {{ old('status') === 'available' ? 'selected' : '' }}>Bo'sh</option>
                    <option value="reserved" {{ old('status') === 'reserved' ? 'selected' : '' }}>Band</option>
                    <option value="occupied" {{ old('status') === 'occupied' ? 'selected' : '' }}>To'liq band</option>
                </select>
                @error('status')
                <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                @enderror
